...
%%depth images
c={'cam0','cam1','cam2'};
filerange=453;%file selecting set of data from 3 sensors
...
%
%[Cam0cam1,ca11m2]
fx=[316.9132  316.8057 333.007548915840];
fy=[325.3394  317.9608 336.677823073012];
cx=[244.2182  266.2595 261.765952021563];
cy=[183.6434  189.0570 191.982091579249];

rel=[1];%11,30,56,77,110;135,160,190,218,250;140,170,195,226,260];
[srel,nrel]=size(rel);
Cylinders=cell(3,nrel);%data%
CyW_dep=cell(3,nrel);%transformed data
CyW_ir=cell(3,nrel);%transformed data
CyW_chess=cell(3,nrel);
CyW_exp2=cell(3,nrel);
CyF_dep=cell(nrel);%fused with depth method;
CyF_ir=cell(nrel);%fused with depth method;
CyF_chess=cell(nrel);
CyF_exp2=cell(nrel);
RR12=R_12_chess;tt12=-R_12_chess*t_12_chess;
...
%                 CyW{1,j}=bsxfun(@plus,RR11*Cylinders{c,j}',tt11);
%                 W1=(bsxfun(@plus,Rii(:,:,1)*Cylinders{1,j}',tii(:,1)));%Rii_P1'*Rii_Pc+Rii_P1'*tii_Pc-Rii_P1'*tii_P1
%                  CyW_dep{1,j}=(bsxfun(@plus,(Rii(:,:,1)'*W1),-Rii(:,:,1)'*tii(:,1)));
                 
%                  CyW_ir{1,i,j}=Cylinders{c,i,j}';
                 CyW_chess{1,j}=Cylinders{c,j}';
...
%                     camPts2 = bsxfun(@minus, Cylinders{c,j}, t_12_chess');
%                     CyW_chess{2,j} = (camPts2 * R_12_chess')';
                
                 CyW_chess{2,j}=bsxfun(@plus,R_12_chess*Cylinders{c,j}',t_12_chess);%R_12_exp1*[t_12_exp1(1);t_12_exp1(2);t_12_exp1(3)/10]);
...
%                     CyW_ir{3,i,j}=bsxfun(@plus,R_13_ir*Cylinders{c,i,j}',t_13_ir);
                    CyW_chess{3,j}=bsxfun(@plus,(R_13_chess)*Cylinders{c,j}',t_13_chess);
...
%         CyF_ir{i,j}=[CyF_ir{i,j};CyW_ir{c,i,j}'];
        CyF_chess{j}=[CyF_chess{j};CyW_chess{c,j}'];
...
%         pcshow(CyW_ir{c,i,j}',Clor{2},'MarkerSize',5);hold on;
        pcshow(CyW_chess{c,i,j}',Clor{c},'MarkerSize',5);hold on;
